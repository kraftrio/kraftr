import { provide, Return, useMetadata } from '@kraftr/core';
...
export function definePath(methodOrPath: string, mayPath?: string): unknown {
  if (
  ...
  return Object.fromEntries(
    keys.map((key) => [
      key,
      provide(`rest.controller:${meta.data.name}.${key}`).with(() => undefined)
    ])
  );
}
